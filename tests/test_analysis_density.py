...
def test_compute_density_profiles_golden(monolith_module):
    m = monolith_module
    golden = _load_golden()
    np.random.seed(409)
...
def test_compute_smoothness_metrics_golden(monolith_module):
    m = monolith_module
    golden = _load_golden()
    np.random.seed(410)
    depth_arr = np.arange(20, 120, 5, dtype=float)
    rho_profiles = 1025 + np.cumsum(np.random.randn(3, len(depth_arr)) * 0.01, axis=1)
    smoothness = m.compute_smoothness_metrics(rho_profiles, depth_arr, "test")
