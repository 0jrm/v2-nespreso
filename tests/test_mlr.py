...
def force_mlr_cpu(monolith_module):
...
def test_prepare_features_golden(monolith_module):
    m = monolith_module
    golden = _load_golden()
    np.random.seed(408)
    inputs = np.random.randn(8, 3)
    X = m.prepare_features(inputs, max_degree=2)
    assert list(X.shape) == golden["prepare_features_shape"]
    assert np.allclose(X[:2, :4], golden["prepare_features_head"], rtol=0, atol=TOL)


def test_fit_pcs_regression_exact_gpu_golden(monolith_module, capsys):
    m = monolith_module
    golden = _load_golden()
...
def test_predict_pcs_exact_gpu_golden(monolith_module):
    ...
    pcs = np.random.randn(8, 4)
    beta = m.fit_pcs_regression_exact_gpu(X, pcs)
    X_new = m.prepare_features(np.random.randn(3, 3), max_degree=2)
    pcs_pred = m.predict_pcs_exact_gpu(beta, X_new)
